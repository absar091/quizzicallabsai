    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.map(category => (
            <Card key={category.id} className="flex flex-col">
                ...
                <CardContent className="mt-auto">
                    <Button asChild>
                        <Link href={`/nts/${category.id}`}>View Syllabus <ArrowRight className="ml-2 h-4 w-4" /></Link>
                    </Button>
                </CardContent>
            </Card>
        ))}
        <Card className="col-span-1 md:col-span-2 lg:col-span-3 bg-primary text-primary-foreground">
            <CardHeader>
                <CardTitle>Full-Length NAT Mock Test</CardTitle>
                <CardDescription className="text-primary-foreground/80">Simulate the real NAT experience (80 MCQs) with our full-length mock test based on your field.</CardDescription>
            </CardHeader>
            <CardContent>
