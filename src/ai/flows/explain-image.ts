...
const explainImageFlow = async (input: ExplainImageInput): Promise<ExplainImageOutput> => {
  // Await the AI instance
  const aiInstance = await ai;
  if (!aiInstance) {
    return {
      explanation: "Image explanation service is currently unavailable. Please try again later."
    };
  }
  ...
  let output: ExplainImageOutput | undefined;
  let retryCount = 0;
  const maxRetries = 2;
